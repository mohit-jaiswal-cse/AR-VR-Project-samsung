...
class ParameterDataset(Dataset):
    ...
    # Read the HDR image from the crop_path
      exr = self.handle.read_hdr(crop_path)

    # Tonemap the HDR image
      input, alpha = self.tone(exr)

    # Convert the tonemapped image to a PyTorch tensor
      input_tensor = self.to_tensor(input)

    # Assign the tensor to training_pair['crop']
      training_pair['crop'] = input_tensor
